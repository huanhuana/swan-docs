...
| bindchooseAddress |EventHandler |   | 否 |用户点击该按钮时，调起用户编辑收货地址原生界面，并在编辑完成后返回用户选择的地址，从返回参数的 detail 中获取，和 swan.chooseAddress 一样的。和 open-type 搭配使用， 使用时机： `open-type="chooseAddress"`。|- |
| bindchooseInvoiceTitle |EventHandler |   | 否 |用户点击该按钮时，选择用户的发票抬头，和 swan.chooseInvoiceTitle 一样的。和 open-type 搭配使用， 使用时机： `open-type="chooseInvoiceTitle"`。|- |
...
<a href="swanide://fragment/5374a49a4e1b12395df1408a66a5f1161580818726105" title="在开发者工具中预览效果" target="_self">在开发者工具中预览效果</a>
